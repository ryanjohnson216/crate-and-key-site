          ...
          <form onSubmit={handleSubmitCheckout} className="space-y-6 max-w-2xl mx-auto">
            ...
            <div className="bg-[#F5F2ED] rounded-2xl p-5 border border-[#EBE3D5] space-y-4">
              ...
              <div className="bg-[#2D2A26] text-[#EBE3D5] rounded-xl p-5 space-y-5 shadow-md">
                ...
                {/* Google Workspace Integration Block */}
                <div className="p-3.5 bg-[#1F1D1A] rounded-xl border border-[#3E362E] space-y-2.5 text-xs">
                  <div className="flex items-center justify-between text-white font-bold">
                    <div className="flex items-center gap-2">
                      <FileSpreadsheet className="w-4 h-4 text-[#FFE033]" />
                    ...
                    {googleUser ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="text-[10px] text-[#A08E79] hover:underline"
                        >
                          Disconnect
                        </button>
                      </div>
                    ) : (
                      <span className="text-[10px] bg-[#3E362E] text-[#A08E79] px-2 py-0.5 rounded">Optional</span>
                    )}
                  </div>

                  {googleUser ? (
                    <div className="space-y-2 pt-1 text-[#EBE3D5]">
                      <label className="flex items-center gap-2 cursor-pointer text-xs">
                        <input
                          type="checkbox"
                          checked={syncToSheets}
                          onChange={(e) => setSyncToSheets(e.target.checked)}
                          className="rounded border-[#5E5449] bg-[#2D2A26] text-[#A08E79] focus:ring-0"
                        />
                        <span className="flex items-center gap-1.5">
                          <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-400" /> Log to Google Sheets
                        </span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer text-xs">
                        <input
                          type="checkbox"
                          checked={syncToGmail}
                          onChange={(e) => setSyncToGmail(e.target.checked)}
                          className="rounded border-[#5E5449] bg-[#2D2A26] text-[#A08E79] focus:ring-0"
                        />
                        <span className="flex items-center gap-1.5">
                          <Mail className="w-3.5 h-3.5 text-blue-400" /> Send email confirmation via Gmail
                        </span>
                      </label>
                      <p className="text-[10px] text-[#A08E79]">
                        Connected as: <span className="text-white font-medium">{googleUser.email}</span>
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-2 text-[11px] text-[#A08E79] pt-1">
                      <p className="leading-snug">Connect Google account to automatically log reservations to Google Sheets and send confirmation emails via Gmail.</p>
                      <button
                        type="button"
                        onClick={handleGoogleLogin}
                        disabled={isSigningInGoogle}
                        className="w-full flex items-center justify-center gap-2 bg-white hover:bg-neutral-100 text-[#2D2A26] font-bold px-3 py-2 rounded-lg border border-neutral-300 text-xs shadow-xs transition"
                      >
